...
public class MomentumScoringService : BackgroundService
{
    ...
    public sealed record ManualAnalysisResult(
        string Status,
        string? Reason = null,
        int? ScoreId = null,
    ...
    public async Task<ManualAnalysisResult> GenerateManualAnalysisAsync(
        string ticker,
        int baselineScoreId,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var latestScore = await db.MomentumScores
            ...
            if (latestScore == null)
            {
                return new ManualAnalysisResult(
                    Status: "skipped",
                    Reason: "No score data found for ticker");
            }

            if (latestScore.Id > baselineScoreId &&
                !string.IsNullOrWhiteSpace(latestScore.AiAnalysis))
            {
                return new ManualAnalysisResult(
                    Status: "skipped",
                    Reason: "Analysis already generated by background scorer",
            ...
            var aiAnalysis = await CallOpenAiAsync(
                ticker,
                signals,
                latestScore.RedditScore,
                latestScore.NewsScore,
                latestScore.VolumeScore,
                latestScore.OptionsScore,
                latestScore.SentimentScore,
            ...
            if (string.IsNullOrWhiteSpace(aiAnalysis))
            {
                return new ManualAnalysisResult(
                    Status: "failed",
                    Reason: "AI analysis could not be generated");
            }

            latestScore.AiAnalysis = aiAnalysis;
            await db.SaveChangesAsync(cancellationToken);
        ...
        {
            _logger.LogError(ex, "Manual analysis failed for {Ticker}", ticker);
            return new ManualAnalysisResult(
                Status: "failed",
                Reason: "Unhandled error while generating analysis");
        }
    }
